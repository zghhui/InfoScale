...
def main():
    ...
    for i in range(num_batches):
        ...
        for n in range(config.num_iters_per_prompt):
            seed = args.seed + n
            set_seed(seed)
            pipeline.unet.set_attn_processor(Attn_CFG_XL(output_prompts))
            ...
            for index, (image, prompt) in enumerate(zip(images, output_prompts)):
                ...
                with open(os.path.join(logging_dir, f"{total_num}.txt"), 'w') as f:
                    attn_obj = pipeline.unet.attn_processors['up_blocks.1.attentions.2.transformer_blocks.1.attn2.processor']
                    entropy_text = getattr(attn_obj, 'entropy_text', 'default_value')
                    entropy_uncond = getattr(attn_obj, 'entropy_uncond', 'default_value')
                    entropy_text_block = getattr(attn_obj, 'entropy_text_block', 'default_value')
                    entropy_uncond_block = getattr(attn_obj, 'entropy_uncond_block', 'default_value')                    

                    f.writelines([prompt, ])
                    torch.set_printoptions(6)
                    print(entropy_text[index])
                    f.writelines([f"\nEntropy_text: {entropy_text[index]}\n", f"Entropy_uncond: {entropy_uncond[index]}\n"])
                    f.writelines([f"\nEntropy_text_block: {entropy_text_block[index]}\n", f"Entropy_uncond_block: {entropy_uncond_block[index]}\n"])
