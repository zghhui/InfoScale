...
from infoscale_sdxl_utils import *
...
def pipeline_processor(
    ...
    def forward(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            # step:4
            last_step_con = None
            last_step_uncon = None
            ds_start, ds_end = 0.01, 0.25
            start_value, end_value = 0.1, 0.35
            num_elements = num_inference_steps
            decrease_steps = dilate_tau

            decrease_amount = (start_value-end_value) / (decrease_steps - 1)
            result_list = [max(0, start_value - i * decrease_amount) for i in range(num_elements)]
            
            
            decrease_amount_ds = (ds_start - ds_end) / (decrease_steps - 1)
            ds_list = [max(ds_end, ds_start - i * decrease_amount_ds) for i in range(num_elements)]
            for i, t in enumerate(timesteps):
                ...
                if do_classifier_free_guidance:
                    ...
                    noise_pred_uncond_clone = noise_pred_uncond.clone()
                    noise_pred_text_clone = noise_pred_text.clone()

                    # step:2
                    xishu = result_list[i]
                    d_s = ds_list[i]
                    d_s = (dilate_tau-i)/dilate_tau
                    if i == 0:
                        last_step_con = noise_pred_text
                        last_step_uncon = noise_pred_uncond
                    elif i <= dilate_tau and i > 0:
                        last_step_con_tmp = last_step_con.clone()
                        last_step_uncon_tmp = last_step_uncon.clone()
                        last_step_con = noise_pred_text.clone()
                        last_step_uncon = noise_pred_uncond.clone()
                        for index in range(batch_size):
                            con_adain = (last_step_con_tmp[index]-torch.mean(last_step_con_tmp[index], dim=0, keepdim=True)) / (torch.std(last_step_con_tmp[index], dim=0, keepdim=True) + 1e-8) * torch.std(noise_pred_text[index], dim=0, keepdim=True) + torch.mean(noise_pred_text[index], dim=0, keepdim=True)
                            uncond_adain = (last_step_uncon_tmp[index]-torch.mean(last_step_uncon_tmp[index], dim=0, keepdim=True)) / (torch.std(last_step_uncon_tmp[index], dim=0, keepdim=True) + 1e-8) * torch.std(noise_pred_uncond[index], dim=0, keepdim=True) + torch.mean(noise_pred_uncond[index], dim=0, keepdim=True)
                            noise_pred_text[index] = freq_mix_3d(noise_pred_text[index], con_adain, d_s=d_s, high_d_s=xishu, t=i)                        
                            noise_pred_uncond[index] = freq_mix_3d(noise_pred_uncond[index], uncond_adain, d_s=d_s, high_d_s=xishu, t=i)
                    # noise_pred_text = noise_pred_text_clone
...
def main():
    ...
    rank = accelerator.process_index  # 获取当前进程的 rank
    ...
    total_num = len(glob.glob(os.path.join(logging_dir, '*.jpg')))
    print(total_num)
